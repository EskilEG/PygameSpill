...
class Player(pygame.sprite.Sprite):
	...
	def move(self, dt):
		...
		#^gjennomsnittet av nedeoverfarten
	...
	def platform_move(self, dt):
		if self.platform:
			if self.platform.direction.x < 0:
				self.hitbox_rect.topleft += self.platform.direction * self.platform.speed * dt * 2
			else:
	...
	def check_contact(self):
			...
			left_rect = pygame.Rect(self.hitbox_rect.topleft + vector(-2,0), (2, self.hitbox_rect.height))
	...
	def collision(self, axis):
		for sprite in self.collision_sprites:
			if sprite.rect.colliderect(self.hitbox_rect):
				if axis == 'horizontal':
					#left
					if self.hitbox_rect.left <= sprite.rect.right and int(self.old_rect.left + 3) >= int(sprite.old_rect.right):
						self.hitbox_rect.left = sprite.rect.right
					#right
					if self.hitbox_rect.right >= sprite.rect.left and int(self.old_rect.right - 3) <= int(sprite.old_rect.left):
						self.hitbox_rect.right = sprite.rect.left
					
				else: #vertical collision
					#top
					if self.hitbox_rect.top <= sprite.rect.bottom and int(self.old_rect.top + 3) >= int(sprite.old_rect.bottom):
						self.hitbox_rect.top = sprite.rect.bottom
						if hasattr(sprite, 'moving'):
							self.hitbox_rect.top += 6
					#bottom
					if self.hitbox_rect.bottom >= sprite.rect.top and int(self.old_rect.bottom - 3) <= int(sprite.old_rect.top):
	...
	def semi_collision(self):
		if not self.timers['platform skip'].active:
			for sprite in self.semi_collision_sprites:
				if sprite.rect.colliderect(self.hitbox_rect):
					if self.hitbox_rect.bottom >= sprite.rect.top and int(self.old_rect.bottom - 3) <= sprite.old_rect.top:
							self.hitbox_rect.bottom = sprite.rect.top
	...
	def flicker(self):
		if self.timers['hit'].active and sin(pygame.time.get_ticks() * 10) >= 0:
